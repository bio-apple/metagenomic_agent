...
from metagenomic_agent.tools.linux_runner import CommandResult, LinuxBioToolRunner

ExecMode = Literal["mock", "local", "conda", "docker"]
...
class ToolContext:
    ...
    def from_config(cls, config: dict[str, Any], outdir: str | Path, mode: str | None = None) -> ToolContext:
        ...
        if raw_mode not in {"mock", "local", "conda", "docker"}:
            # treat legacy alias
            raw_mode = "mock"
        ...
        # Prefer /dev/shm copies when configured
        ...
        if linux_cfg.get("prefer_shm", True) and Path(shm).exists():
            for key in ("kraken2_db", "gtdb", "host_index"):
                ...
                if shm_candidate.exists() and any(shm_candidate.iterdir()) if shm_candidate.is_dir() else shm_candidate.exists():
    ...
    def run_tool(self, tool: str, argv: list[str], timeout: int | None = None, check: bool = False) -> CommandResult:
        """Run via conda (production) or local PATH."""
        if self.mode == "conda":
            runner = LinuxBioToolRunner(
                conda_env=self.conda_envs.get(tool, "metagenomics"),
                use_conda=True,
            )
            result = runner.run_command(argv, timeout=timeout, check=check)
            self.last_result = result
            return result
        # local
        try:
            rr = run_command(argv, check=check)
            result = CommandResult(
                status="success" if rr.ok else "failed",
                stdout=rr.stdout,
                stderr=rr.stderr,
                returncode=rr.returncode,
                command=rr.command,
                error=None if rr.ok else rr.stderr,
                classified=None if rr.ok else "logic",
            )
        except RuntimeError as exc:
            result = CommandResult(status="failed", error=str(exc), classified="logic", command=" ".join(argv))
            if check:
                raise
        self.last_result = result
        return result
