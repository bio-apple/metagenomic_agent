...
def test_graph_dryrun_mock(tmp_path: Path):
    ...
    assert (out / "planner" / "planner_plan.json").exists()
    assert (out / "executor" / "submit.slurm").exists()
    assert (out / "executor" / "submit.pbs").exists()
    assert (out / "executor" / "submit.sge").exists()
